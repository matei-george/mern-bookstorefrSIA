   ...
   return (
      <div className="admin-products-container">
         ...
         <div className="admin-filters-section">
            ...
            />
            <select value={filters.status} onChange={(e) => handleStatusChange(e.target.value)} className="admin-status-filter">
         ...
         {loading && products.length > 0 && <div className="admin-search-loading">Se caută...</div>}

         {showForm && (
            <div className="admin-modal-overlay">
               <div className="admin-modal-content">
                  <h2 className="admin-modal-title">{editingProduct ? "Editare Produs" : "Produs Nou"}</h2>
         ...
         <div className="admin-products-grid">
            {products.length === 0 ? (
               ...
               products.map((product) => (
                  <div key={product.id} className={`admin-product-card ${!product.isActive ? "admin-product-inactive" : ""}`}>
                     <div className="admin-product-image">
                        {product.imageUrl ? (
                           <img src={product.imageUrl} alt={product.title} className="admin-product-img" />
                        ) : (
                     ...
                     <div className="admin-product-info">
                        ...
                        <div className="admin-product-pricing">
                           {product.discountPrice ? (
                              <>
                                 <span className="admin-product-original-price">{product.price} RON</span>
                                 <span className="admin-product-discount-price">{product.discountPrice} RON</span>
                                 <span className="admin-product-discount-badge">-{Math.round((1 - product.discountPrice / product.price) * 100)}%</span>
                              </>
                           ) : (
                              <span className="admin-product-price">{product.price} RON</span>
                           )}
                        </div>
                     ...
                     <div className="admin-product-actions">
                        <button onClick={() => handleEdit(product)} className="admin-btn admin-btn-edit">
                           Editare
                        </button>
                        <button
                           onClick={() => handleToggleStatus(product.id, product.isActive)}
                           className={product.isActive ? "admin-btn admin-btn-warning" : "admin-btn admin-btn-success"}
                        >
                     ...
                     {product.featured && <div className="admin-featured-badge">Recomandat</div>}
                  </div>
               ))
            )}
         </div>
